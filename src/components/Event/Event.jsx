...
export default function Event() {
    const imageHolderRef = useRef(null); // Reference to the image-holder
    let idx = 0;

    useEffect(() => {
        const interval = setInterval(() => {
            if (imageHolderRef.current) {
                const img = imageHolderRef.current.querySelector("img");
                if (img) {
    ...
    return (
        <div className="w-full h-min rounded-lg px-12 pb-12 bg-white/5 backdrop-blur-xl shadow-[0_0_22px_rgba(0,0,0,0.2)] text-[#d5d5d5] text-[1.5rem] text-center max-xs:scale-[0.9]">
            ...
            <div className="flex justify-evenly flex-wrap">
                <div className="about-event min-w-[300px] h-96 text-[1.5rem] p-8 text-left bg-[rgba(0,0,0,0.42)] rounded-[10px] mb-[1rem] overflow-y-auto basis-[45%] max-xs:text-base mini:text-xs">
                    <p className="text-[#E2E8F0] font-kode text-[1.25rem] font-normal mt-[2rem] leading-[1.75rem] max-xs:text-base mini:text-xs">
                        Excalibur is a software development team event.
                        Students from the first, second, pre-final years of all branches and programs are eligible to participate.
                ...
                <div className="min-w-[300px] h-96 text-[1.5rem] p-8 text-center bg-[rgba(0,0,0,0.427)] rounded-[10px] basis-[45%]">
                    <p>Excalibur Glimpses</p>
                    <div ref={imageHolderRef} className="image-holder my-8 h-[80%] text-center flex">
                        <img
                            src={imgUrls[0]}
                            alt="Excalibur Glimpses"
